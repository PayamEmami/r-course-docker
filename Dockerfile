...
RUN gpg --keyserver keyserver.ubuntu.com --recv-key E298A3A825C0D65DFD57CBB651716619E084DAB9 \
 && gpg -a --export E298A3A825C0D65DFD57CBB651716619E084DAB9 | sudo apt-key add - 
RUN  apt-get update && apt-get install -yq --no-install-recommends r-cran-curl

RUN R -e 'install.packages("BioManager"); BiocManager::install(c("tidyverse","lubridate","nycflights13","bookdown","captioner","formattable"))'
RUN R -e 'BiocManager::install(c("kableExtra","knitr","leaflet","lubridate","markdown","remotes","rmarkdown","xaringan","yaml"))'
RUN  apt-get update && apt-get install -yq  libcurl4-openssl-dev
RUN R -e 'BiocManager::install("devtools")'
RUN R -e 'library(devtools); install_github(repo="rstudio/fontawesome")'
RUN R -e 'library(devtools); install_github(repo="mkierczak/mkteachr")' 
